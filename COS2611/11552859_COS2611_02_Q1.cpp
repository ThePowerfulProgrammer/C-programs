...
// this is the nodes that will be used in our BST?
class TreeNode
{
...
{
public:
    TreeNode* root; // value = 0,left,right = NULL
    ...
    int heightOfTree(TreeNode* r) {
        ...
        int lH = heightOfTree(r->left);
        int rH = heightOfTree(r->right);
    ...
    void printPreorder(TreeNode* r) //(current node, Left, Right)
    {
		...
		if (r != NULL)
            {
                cout << r->value << " ";
                printPreorder(r->left); // print leftmost and move back
                printPreorder(r->right); // print rightmost and move back
            }
